...
def test_block_size_dataloader(split: str, block_size: int, batch_size: int):
    ...
    dl = data.fetch_dataset_loader(split, encoder, batch_size, block_size)
    assert next(iter(dl)).shape == (batch_size, block_size)

    ddp1 = data.DDPConfig(0, 0, 2)
    ddp2 = data.DDPConfig(1, 1, 2)
    ...
    assert (next(iter(dl1)) != (next(iter(dl2)))).any()
